...
export default function AdminPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!body.name?.trim() || !body.imageUrl?.trim()) {
      const metadataResponse = await fetch('/api/product-metadata', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ url: body.sourceUrl }) });
      const metadata = await metadataResponse.json() as { title?: string; imageUrl?: string; error?: string };
      if (!metadataResponse.ok) { setBusy(false); setMessage(`상품명 또는 대표 이미지가 비어 있어 자동 조회를 시도했지만 실패했습니다. ${metadata.error || '수동으로 입력해 주세요.'}`); return; }
      if (!body.name?.trim() && metadata.title) body.name = metadata.title;
      if (!body.imageUrl?.trim() && metadata.imageUrl) body.imageUrl = metadata.imageUrl;
    }
