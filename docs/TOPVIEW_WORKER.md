...
3. Topview MCP에서 board/config/upload/generate/query 순서로 처리한다. 제품 이미지가 있으면 반드시 typed image input으로 업로드하고 `image_to_video`를 사용한다. 이미지가 없을 때만 `text_to_video`를 사용한다.
