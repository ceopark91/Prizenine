...
function setupSystem() {
  ...
  if (queue.getLastRow() === 0) queue.appendRow(['job_id','상품URL','상태','접수시각','처리시각','제품번호','오류','단계']);
...
function reclaimStaleProcessing_() {
  var q = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(QUEUE_SHEET);
  if (!q || q.getLastRow() < 2) return;
  var rows = q.getRange(2, 1, q.getLastRow() - 1, 7).getValues();
  var now = new Date(), limit = 15 * 60 * 1000;
  rows.forEach(function(r, i) {
    if (r[2] !== 'processing') return;
    var started = r[4] instanceof Date ? r[4] : r[3];
    if (started && now.getTime() - new Date(started).getTime() > limit) {
      q.getRange(i + 2, 3).setValue('pending');
      q.getRange(i + 2, 7).setValue('자동 복구: 작업자 중단 후 재시도');
    }
  });
}
...
function enqueue_(url, source) {
  ...
  if (q.getLastRow() === 0) q.appendRow(['job_id','상품URL','상태','접수시각','처리시각','제품번호','오류','단계']);
  ...
  var id=Utilities.getUuid(); q.appendRow([id,url,'pending',new Date(),'','','','대기']);
...
function complete_(data) {
  ...
  for (var j = 0; j < categories.length; j++) {
    if (String(categories[j][0]).trim() === '') { writeRow = j + 2; break; }
    writeRow = j + 3;
  }
...
function claim_(id){
  ...
  for(var i=0;i<ids.length;i++)if(ids[i][0]===id){var state=q.getRange(i+2,3).getValue();if(state!=='pending')return {ok:false,status:state};q.getRange(i+2,3).setValue('processing');q.getRange(i+2,8).setValue('상품 페이지 열기');return {ok:true,status:'processing',jobId:id,url:q.getRange(i+2,2).getValue(),stage:'상품 페이지 열기'};}
  return {ok:false,error:'job not found'};
}

function setStage_(id, stage) {
  var q=SpreadsheetApp.getActiveSpreadsheet().getSheetByName(QUEUE_SHEET); if(!q)return {ok:false};
  var ids=q.getRange(2,1,Math.max(q.getLastRow()-1,1),1).getValues();
  for(var i=0;i<ids.length;i++) if(ids[i][0]===id){q.getRange(i+2,8).setValue(stage); return {ok:true,jobId:id,stage:stage};}
  return {ok:false,error:'job not found'};
}
